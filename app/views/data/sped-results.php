<section class="section">
    <div class="container">
        <div class="section-header">
            ...
            <h2 class="section-title">SPED Results Browser</h2>
            <p class="section-subtitle">Special education outcomes — graduation rates, dropout rates, inclusion data.</p>
        </div>
        ...
        <?php else: ?>
            ...
            <div style="overflow-x:auto;">
                <table class="data-table" style="width:100%;border-collapse:collapse;">
                    <thead><tr><th>District</th><th>Year</th><th>Grad Rate %</th><th>Dropout Rate %</th><th>Full Inclusion %</th><th>Parent Involvement</th></tr></thead>
                    <tbody>
                        <?php foreach ($rows as $r): ?>
                        <tr>
                            <td><?= h($r['district_name']) ?></td>
                            <td><?= h($r['school_year']) ?></td>
                            <td><?= number_format((float)($r['sped_grad_rate'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((float)($r['sped_dropout_rate'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((float)($r['lre_full_incl_pct'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((float)($r['parent_involve_pct'] ?? 0), 1) ?>%</td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
