<section class="section">
    <div class="container">
        <div class="section-header">
            ...
            <h2 class="section-title">Enrollment Demographics Browser</h2>
            <p class="section-subtitle">Enrollment by SPED status, economic disadvantage, English learner, and high-needs across MA districts.</p>
        </div>
        ...
        <?php else: ?>
            ...
            <div style="overflow-x:auto;">
                <table class="data-table" style="width:100%;border-collapse:collapse;">
                    <thead><tr><th>District</th><th>Year</th><th>High Needs</th><th>High Needs %</th><th>EL</th><th>EL %</th><th>Low Income</th><th>Low Income %</th><th>SPED</th><th>SPED %</th></tr></thead>
                    <tbody>
                        <?php foreach ($rows as $r): ?>
                        <tr>
                            <td><?= h($r['district_name']) ?></td>
                            <td><?= h($r['school_year']) ?></td>
                            <td><?= number_format((int)($r['high_needs_num'] ?? 0)) ?></td>
                            <td><?= number_format((float)($r['high_needs_pct'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((int)($r['el_num'] ?? 0)) ?></td>
                            <td><?= number_format((float)($r['el_pct'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((int)($r['low_income_num'] ?? 0)) ?></td>
                            <td><?= number_format((float)($r['low_income_pct'] ?? 0), 1) ?>%</td>
                            <td><?= number_format((int)($r['sped_num'] ?? 0)) ?></td>
                            <td><?= number_format((float)($r['sped_pct'] ?? 0), 1) ?>%</td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
