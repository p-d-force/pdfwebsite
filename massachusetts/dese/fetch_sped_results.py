#!/usr/bin/env python3
"""DESE Special Education Results Scraper — SQLite + MariaDB seed output"""
...
BASE_URL = "https://profiles.doe.mass.edu/statereport/special_education.aspx"
YEARS = ["2018", "2019", "2020", "2021", "2022", "2023", "2024", "2025"]
TABLE = "sped_results"
DATASET = "sped_results"
...
def init_db(db_path):
    ...
    c.execute(f"""CREATE TABLE IF NOT EXISTS {TABLE} (
        ...
        school_year TEXT,
        sped_grad_rate REAL,
        sped_dropout_rate REAL,
        lre_full_incl_pct REAL,
        parent_involve_pct REAL,
...
class TableParser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        ...
        if self.in_row and tag == "tr":
            self.in_row = False
            if len(self.current_row) >= 6 and self.skipped:
    ...
    for r in rows:
        try:
            records.append({
                ...
                "sped_grad_rate": parse_float(r[2]) if len(r) > 2 else None,
                "sped_dropout_rate": parse_float(r[3]) if len(r) > 3 else None,
                "lre_full_incl_pct": parse_float(r[4]) if len(r) > 4 else None,
                "parent_involve_pct": parse_float(r[5]) if len(r) > 5 else None,
            })
        except Exception as e:
            print(f"  skip row: {e}")
    return records
...
def write_sqlite(db_path, records):
    ...
    sql = f"""INSERT OR REPLACE INTO {TABLE}
        (district_name, district_code, school_year, sped_grad_rate, sped_dropout_rate,
         lre_full_incl_pct, parent_involve_pct)
        VALUES (?,?,?,?,?,?,?)"""
    for rec in records:
        c.execute(sql, (
            rec["district_name"], rec["district_code"], rec["school_year"],
            rec["sped_grad_rate"], rec["sped_dropout_rate"],
            rec["lre_full_incl_pct"], rec["parent_involve_pct"]
        ))
    conn.commit()
    conn.close()
...
def generate_seed_sql(output_path):
    ...
    c.execute(f"""SELECT district_name, district_code, school_year,
        sped_grad_rate, sped_dropout_rate, lre_full_incl_pct, parent_involve_pct
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("-- DESE Special Education Results Data\n")
        ...
        f.write("    sped_grad_rate DECIMAL(5,1) NULL,\n")
        f.write("    sped_dropout_rate DECIMAL(5,1) NULL,\n")
        f.write("    lre_full_incl_pct DECIMAL(5,1) NULL,\n")
        f.write("    parent_involve_pct DECIMAL(5,1) NULL,\n")
        ...
        if rows:
            ...
            vals = [f"({esc(r[0])},{esc(r[1])},{esc(r[2])},{esc(r[3])},{esc(r[4])},{esc(r[5])},{esc(r[6])})" for r in rows]
            f.write(",\n".join(vals) + ";\n")
...
def main():
    print("=" * 60)
    print("DESE Special Education Results Scraper")
    print(f"Years: {YEARS[0]} through {YEARS[-1]} (single-year URL format)")
    print("=" * 60)
    init_db(DB_PATH)
    ...
    if all_rows:
        output_path = os.path.join(OUTPUT_DIR, "seed_sped.sql")
